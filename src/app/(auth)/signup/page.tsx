...
export default function SignupPage() {
  ...
  if (done) {
    return (
      ...
      >
        <div
          className="w-full max-w-md rounded-2xl overflow-hidden text-center p-10"
          style={{ border: '2px solid #000', boxShadow: '6px 6px 0px 0px #000' }}
        >
          <div
            className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-5"
            style={{ background: '#F5E642', border: '2px solid #000', boxShadow: '3px 3px 0px 0px #000' }}
          >
            <CheckCircle2 className="w-7 h-7 text-black" />
          </div>
          <h2 className="text-[26px] font-extrabold tracking-tight text-black mb-2">Check your email</h2>
          <p className="text-[14px] text-black/55 font-medium mb-7">
            We sent a confirmation link to <strong className="text-black">{email}</strong>. Click it to activate your account.
          </p>
          <button
            onClick={() => router.push('/login')}
            className="inline-flex items-center gap-2 px-6 py-3 text-[14px] font-bold text-black"
            style={{ border: '2px solid #000', borderRadius: '12px', background: '#F0EDE8', boxShadow: '3px 3px 0px 0px #000' }}
          >
