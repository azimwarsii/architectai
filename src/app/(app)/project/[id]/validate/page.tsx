...
const FOCUS_PILLS = ['B2B', 'B2C', 'Consumer', 'Enterprise', 'Niche market', 'High WTP']
...
// Tag symbol map — clean, no color
const TAG_SYMBOL: Record<string, string> = { Validated: '✓', Risk: '!', Opportunity: '+', Gap: '○' }
...
export default function ValidatePage() {
  ...
  // Load project + check sessionStorage autostart
  useEffect(() => {
    supabase.from('projects').select('raw_idea, domain, tinyfish_report').eq('id', projectId).single()
      .then(({ data }) => {
        if (!data) return
        if (data.raw_idea) setIdea(data.raw_idea)
        ...
        if (r?.type === 'validation') {
          ...
          if (r.results?.length) {
            setSessions([{ id: '0', label: 'Validation scan', results: r.results as ValidationResult[] }])
            setPhase('active')
            return
          }
        }
        ...
        if (typeof window !== 'undefined') {
          ...
          if (raw) {
            ...
            if (p.autostart && p.mode === 'validate' && p.idea) {
              sessionStorage.removeItem('tf-scan-params')
              autostartFired.current = true
              setIdea(p.idea)
              if (p.domain) setDomain(p.domain)
              runScanDirect('initial', 'Validation scan', p.idea, p.domain || '')
            }
          }
        }
      })
  ...
  function buildDeeperLabel() {
    const parts: string[] = []
    if (selectedSources.length) parts.push(selectedSources.slice(0, 2).join(', '))
  ...
  async function runScanDirect(mode: 'initial' | 'refined' | 'deeper', label: string, scanIdea: string, scanDomain: string) {
    setScanning(true)
    setActiveScanLines([])
    setPhase('active')
    setPanel('none')
    ...
    if (mode === 'initial') {
      await supabase.from('projects').update({ raw_idea: scanIdea, domain: scanDomain || null, name: scanIdea.slice(0, 60), status: 'questioning' }).eq('id', projectId)
    }

    const res = await fetch('/api/tinyfish/validate', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        idea: scanIdea, domain: scanDomain, mode,
    ...
    let buffer = ''; const newResults: ValidationResult[] = []
    while (true) {
      ...
      for (const line of parts) {
        ...
        else if (data.type === 'results') newResults.push(...data.results)
      }
    }

    const newSession: ScanSession = { id: `${Date.now()}`, label, results: newResults }
    setSessions(prev => {
      const updated = [...prev, newSession]
      supabase.from('projects').update({
        tinyfish_report: { type: 'validation', sessions: updated, completed_at: new Date().toISOString() },
  ...
  function runScan(mode: 'initial' | 'refined' | 'deeper', label: string) {
    runScanDirect(mode, label, idea, domain)
  }
        ...
        <div className="max-w-2xl mx-auto p-8">
          ...
          {phase === 'input' && (
            <>
              ...
              <h1 style={{ fontSize: 28, fontWeight: 900, color: '#000', letterSpacing: '-0.02em', marginBottom: 6 }}>Tell us your idea</h1>
              <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.45)', fontWeight: 500, marginBottom: 24 }}>TinyFish will scan for real demand signals, competitor gaps, and user pain points.</p>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div>
                  <label style={{ ...sLabel, display: 'block', marginBottom: 7 }}>Your idea</label>
                  <textarea rows={4} style={{ ...inputBase, resize: 'none' }} placeholder="e.g. A real-time tracking app for peer-to-peer package delivery between travellers…" value={idea} onChange={e => setIdea(e.target.value)}
                    onFocus={e => { e.currentTarget.style.boxShadow = '3px 3px 0 0 #000' }} onBlur={e => { e.currentTarget.style.boxShadow = 'none' }} />
                </div>
                <div>
                  <label style={{ ...sLabel, display: 'block', marginBottom: 7 }}>Domain / market <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional)</span></label>
                  <input style={inputBase} placeholder="e.g. logistics, edtech, fintech…" value={domain} onChange={e => setDomain(e.target.value)} onKeyDown={e => e.key === 'Enter' && runScan('initial', 'Validation scan')}
                    onFocus={e => { e.currentTarget.style.boxShadow = '3px 3px 0 0 #000' }} onBlur={e => { e.currentTarget.style.boxShadow = 'none' }} />
                </div>
                <button onClick={() => runScan('initial', 'Validation scan')} disabled={!idea.trim()}
                  style={{ width: '100%', padding: '12px', borderRadius: 12, border: '2px solid #000', background: idea.trim() ? '#F4520E' : '#ddd', color: idea.trim() ? '#fff' : '#999', fontSize: 14, fontWeight: 800, cursor: idea.trim() ? 'pointer' : 'not-allowed', fontFamily: TF, boxShadow: idea.trim() ? '4px 4px 0 0 #000' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all .14s' }}
                  onMouseEnter={e => { if (idea.trim()) { (e.currentTarget as HTMLElement).style.boxShadow = '6px 6px 0 0 #000'; (e.currentTarget as HTMLElement).style.transform = 'translate(-2px,-2px)' } }}
                  ...
                  Start validation scan <ArrowRight style={{ width: 16, height: 16 }} />
                </button>
              </div>
            </>
          )}

          {/* ── ACTIVE phase: sessions stacked ── */}
          {phase === 'active' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              ...
              {sessions.map((session, si) => (
                <div key={session.id}>
                  ...
                  {session.results.map((r, ri) => (
                    <div key={ri} style={{ display: 'flex', gap: 16, padding: '14px 0', borderBottom: '1px solid rgba(0,0,0,0.07)' }}>
                      <span style={{ fontSize: 14, fontWeight: 800, color: '#bbb', fontFamily: MONO, flexShrink: 0, width: 20, paddingTop: 1 }}>
                        {TAG_SYMBOL[r.tag] || '·'}
                      </span>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                          <span style={{ fontSize: 14, fontWeight: 700, color: '#000', lineHeight: 1.3 }}>{r.title}</span>
                          <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: '#888', fontFamily: MONO, flexShrink: 0 }}>{r.tag}</span>
                        </div>
                        <div style={{ fontSize: 13, color: '#444', lineHeight: 1.6 }}>{r.body}</div>
                        {r.evidence_count != null && (
                          <div style={{ fontSize: 11, color: '#aaa', fontFamily: MONO, fontWeight: 600, marginTop: 6 }}>{r.evidence_count} evidence points</div>
                        )}
                      </div>
                    </div>
                  ))}
                ...
                <div>
                  ...
                  <div style={{ background: '#111', borderRadius: 10, padding: '12px 16px', fontFamily: MONO, fontSize: 11, lineHeight: 2, minHeight: 80, border: '2px solid #000', boxShadow: '3px 3px 0 0 #000' }}>
                    ...
                    {activeScanLines.map((line, i) => (
                      <div key={i} style={{ color: line.kind === 'success' ? '#4ade80' : line.kind === 'warning' ? '#fbbf24' : line.kind === 'error' ? '#f87171' : 'rgba(255,255,255,0.55)' }}>
                        {line.text}
                      </div>
                    ))}
                  </div>
              ...
              {/* Continue to spec builder + Refine / Dig deeper */}
              {!scanning && allResults.length > 0 && (
                <>
                  {/* Continue CTA */}
                  <div style={{ marginTop: 28, marginBottom: 10 }}>
                    <button onClick={() => router.push(`/project/${projectId}/canvas`)}
                      style={{ width: '100%', padding: '13px', borderRadius: 12, border: '2px solid #000', background: '#000', color: '#fff', fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: TF, boxShadow: '4px 4px 0 0 #F5E642', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all .14s' }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = '6px 6px 0 0 #F5E642'; (e.currentTarget as HTMLElement).style.transform = 'translate(-2px,-2px)' }}
                      ...
                      Continue to spec builder <ArrowRight style={{ width: 16, height: 16 }} />
                    </button>
                  </div>
                  ...
                  <div style={{ marginTop: 14 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      <button onClick={() => togglePanel('refine')}
                        style={{ border: '2px solid #000', borderRadius: 12, padding: '14px 16px', cursor: 'pointer', textAlign: 'left', fontFamily: TF, background: panel === 'refine' ? '#F5E642' : '#fff', boxShadow: panel === 'refine' ? '4px 4px 0 0 #000' : '3px 3px 0 0 #000', transform: panel === 'refine' ? 'translate(-1px,-1px)' : 'none', transition: 'all .15s' }}
                        onMouseEnter={e => { if (panel !== 'refine') { const el = e.currentTarget as HTMLElement; el.style.background = '#F5E642'; el.style.transform = 'translate(-1px,-1px)'; el.style.boxShadow = '4px 4px 0 0 #000' } }}
                        onMouseLeave={e => { if (panel !== 'refine') { const el = e.currentTarget as HTMLElement; el.style.background = '#fff'; el.style.transform = 'none'; el.style.boxShadow = '3px 3px 0 0 #000' } }}>
                        <div style={{ fontSize: 18, marginBottom: 5 }}>⟳</div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: '#000', marginBottom: 2 }}>Refine</div>
                        <div style={{ fontSize: 11, color: '#555', lineHeight: 1.4 }}>Narrow focus, geography or market size</div>
                      </button>
                      <button onClick={() => togglePanel('dig')}
                        style={{ border: '2px solid #000', borderRadius: 12, padding: '14px 16px', cursor: 'pointer', textAlign: 'left', fontFamily: TF, background: panel === 'dig' ? '#F5E642' : '#fff', boxShadow: panel === 'dig' ? '4px 4px 0 0 #000' : '3px 3px 0 0 #000', transform: panel === 'dig' ? 'translate(-1px,-1px)' : 'none', transition: 'all .15s' }}
                        onMouseEnter={e => { if (panel !== 'dig') { const el = e.currentTarget as HTMLElement; el.style.background = '#F5E642'; el.style.transform = 'translate(-1px,-1px)'; el.style.boxShadow = '4px 4px 0 0 #000' } }}
                        onMouseLeave={e => { if (panel !== 'dig') { const el = e.currentTarget as HTMLElement; el.style.background = '#fff'; el.style.transform = 'none'; el.style.boxShadow = '3px 3px 0 0 #000' } }}>
                        <div style={{ fontSize: 18, marginBottom: 5 }}>↓</div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: '#000', marginBottom: 2 }}>Dig deeper</div>
                        <div style={{ fontSize: 11, color: '#555', lineHeight: 1.4 }}>Pick a source or signal to investigate</div>
                      </button>
                    </div>
                  </div>
                </>
                  ...
                  <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    {digTab === 'question' && (
                      <>
                        ...
                        <textarea rows={3} placeholder="e.g. What does the competitive landscape look like in Europe?" value={deepQuestion} onChange={e => setDeepQuestion(e.target.value)}
